...
from toolbox.registrable.params import Params
...
def takes_arg(obj, arg: str) -> bool:
    ...
    if inspect.isclass(obj):
        ...
        raise AssertionError('object {} is not callable'.format(obj))
...
def construct_arg(cls: Type[T],
    ...
    # The next case is when the parameter type is itself constructible from_params.
        ...
        if name in params:
            ...
            raise AssertionError(f"expected key {name} for {cls.__name__}")
        else:
            return default
    ...
    # which it creates by instantiating each value from_params and returning the resulting structure.
        ...
        raise AssertionError(f"Failed to construct argument {name} with type {annotation}")
    else:
...
class FromParams(object):
    ...
    def from_params(cls: Type[T], params: Params, **extras) -> T:
        ...
        """
        # import here to avoid circular imports
        from toolbox.registrable.registrable import Registrable
